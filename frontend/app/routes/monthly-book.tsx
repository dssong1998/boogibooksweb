...
export function meta({}: Route.MetaArgs) {
  return [
    { title: '부기북스 - 이달의 책' },
    {
      name: 'description',
      content:
        '부기가 고심 끝에 고른 이 달의 책을 추천합니다. 페이지를 열어 이달의 책을 확인하세요.',
    },
  ];
}
...
export default function MonthlyBook() {
  ...
  const [selectedBook, setSelectedBook] = useState<MonthlyBookData | null>(
    null,
  );
  ...
  const currentYear =
    monthlyBooks.length > 0 ? monthlyBooks[0].year : new Date().getFullYear();
  const currentMonth =
    monthlyBooks.length > 0 ? monthlyBooks[0].month : new Date().getMonth() + 1;
              ...
              <div className='bg-gradient-to-r from-amber-500 to-orange-500 rounded-lg shadow-md p-6 text-white'>
                ...
                {currentTopic && (
                  <p className='text-lg opacity-90'>
                    이달의 주제:{' '}
                    <span className='font-bold'>{currentTopic}</span>
                  </p>
                )}
