...
export function meta({}: Route.MetaArgs) {
  return [
    ...
    {
      name: 'description',
      content: '토스를 이용하여 부기북스 이벤트 참가 비용을 결제하세요 !',
    },
  ];
}
...
export default function Payment() {
    ...
    const loadData = async () => {
      ...
      try {
        ...
        if (userIdParam && me.id !== userIdParam) {
          setLoadError(
            '이 결제 링크는 다른 계정용입니다. 해당 계정으로 로그인해 주세요.',
          );
          setIsLoading(false);
          return;
        }
  ...
  const handleTossPayment = async () => {
    ...
    try {
      const skipEventPaidConfirm = target.paymentKind === 'EVENT' && isPaid;
        ...
        <div className='bg-white dark:bg-gray-800 rounded-lg shadow-md p-8'>
            ...
            {target.paymentKind === 'BOOGI_OUT' &&
              target.settlementMode === 'COIN_GAIN' && (
                <p className='text-sm text-gray-600 dark:text-gray-400 mb-4'>
                  정산 방식: 코인 획득 — 아래 계좌는 송금 안내가 없을 때 공용
                  안내용입니다.
                </p>
              )}
          ...
          <div className='mb-6 p-4 bg-amber-50 dark:bg-amber-900/20 rounded-lg border border-amber-200 dark:border-amber-800'>
            ...
            <ul className='text-sm text-amber-700 dark:text-amber-400 space-y-1'>
              <li>
                • 입금자명은 <strong>{userName || '본인 이름'}</strong>으로
                해주세요.
              </li>
              <li>
                • 송금 후 아래 &quot;직접 송금 완료&quot; 버튼을 눌러주세요.
              </li>
            </ul>
          </div>
